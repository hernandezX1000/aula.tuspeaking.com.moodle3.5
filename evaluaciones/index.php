...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($action === 'exportar_csv' && $es_cesce){
        header("Content-Type: text/csv; charset=utf-8");
        header("Content-Disposition: attachment; filename=Evaluaciones_Cesce_25.2_" . date("Y-m-d") . ".csv");
        echo "\xEF\xBB\xBF"; // BOM UTF-8
        $out = fopen("php://output", "w");
        fputcsv($out, ["Nombre","Email","Grupo","Nivel","Profesor","Nota Plataforma","Nota Oral","Nota Clase","Nota Final","Clases Previstas","Clases Asistidas","Asistencia %","Diploma","Criterio"], ";");
        $rows = $pdo->query("SELECT CONCAT(e.firstname,' ',e.lastname) as nombre, e.email, c.grupo, c.nivel, c.profesor, c.nota_plataforma, c.nota_oral, c.nota_clase, c.nota_final, c.clases_previstas, c.clases_asistidas, c.asistencia_porcentaje, c.diploma FROM mdl_cesce_calificaciones c LEFT JOIN mdl_cesce_empleados e ON LOWER(e.email)=LOWER(c.empleado_email) WHERE c.edicion='25.2' ORDER BY c.nivel, e.lastname")->fetchAll(PDO::FETCH_ASSOC);
        foreach($rows as $r){
            $criterio = $r["diploma"]=="Superacion" ? "Nota≥5 y Asist≥50%" : ($r["diploma"]=="Participacion" ? "Asist≥50%" : "Asist<50%");
            fputcsv($out, [$r["nombre"],$r["email"],$r["grupo"],$r["nivel"],$r["profesor"],$r["nota_plataforma"]?:"-",$r["nota_oral"]?:"-",$r["nota_clase"]?:"-",round($r["nota_final"],2),$r["clases_previstas"],$r["clases_asistidas"],round($r["asistencia_porcentaje"],0)."%",$r["diploma"],$criterio], ";");
        }
        fclose($out);
        exit;
    }
    
    if($action === 'exportar_xlsx' && $es_cesce){
        $rows = $pdo->query("SELECT CONCAT(e.firstname,' ',e.lastname) as nombre, e.email, c.grupo, c.nivel, c.profesor, c.nota_plataforma, c.nota_oral, c.nota_clase, c.nota_final, c.clases_previstas, c.clases_asistidas, c.asistencia_porcentaje, c.diploma FROM mdl_cesce_calificaciones c LEFT JOIN mdl_cesce_empleados e ON LOWER(e.email)=LOWER(c.empleado_email) WHERE c.edicion='25.2' ORDER BY c.nivel, e.lastname")->fetchAll(PDO::FETCH_ASSOC);
        $html = "<html><head><meta charset='utf-8'></head><body><table border='1'>";
        $html .= "<tr style='background:#667eea;color:white;font-weight:bold'><td>Nombre</td><td>Email</td><td>Grupo</td><td>Nivel</td><td>Profesor</td><td>Nota Plat.</td><td>Nota Oral</td><td>Nota Clase</td><td>Nota Final</td><td>Clases Prev.</td><td>Clases Asist.</td><td>Asistencia %</td><td>Diploma</td><td>Criterio</td></tr>";
        foreach($rows as $r){
            $criterio = $r["diploma"]=="Superacion" ? "Nota≥5 y Asist≥50%" : ($r["diploma"]=="Participacion" ? "Asist≥50%" : "Asist<50%");
            $bg = $r["diploma"]=="Superacion" ? "#d1fae5" : ($r["diploma"]=="Participacion" ? "#fef3c7" : "#fee2e2");
            $html .= "<tr style='background:$bg'><td>{$r["nombre"]}</td><td>{$r["email"]}</td><td>{$r["grupo"]}</td><td>{$r["nivel"]}</td><td>{$r["profesor"]}</td><td>".($r["nota_plataforma"]?:"-")."</td><td>".($r["nota_oral"]?:"-")."</td><td>".($r["nota_clase"]?:"-")."</td><td>".round($r["nota_final"],2)."</td><td>{$r["clases_previstas"]}</td><td>{$r["clases_asistidas"]}</td><td>".round($r["asistencia_porcentaje"],0)."%</td><td>{$r["diploma"]}</td><td>$criterio</td></tr>";
        }
        $html .= "</table></body></html>";
        header("Content-Type: application/vnd.ms-excel; charset=utf-8");
        header("Content-Disposition: attachment; filename=Evaluaciones_Cesce_25.2_" . date("Y-m-d") . ".xls");
        echo $html;
        exit;
    }
...
<div class="card">
    ...
    <div class="actions-bar">
        ...
        <?php if($es_cesce):?><form method="post" style="display:inline"><input type="hidden" name="action" value="exportar_csv"><button type="submit" class="btn btn-success">📥 Exportar CSV</button></form>
        <form method="post" style="display:inline"><input type="hidden" name="action" value="exportar_xlsx"><button type="submit" class="btn btn-primary">📊 Exportar Excel</button></form><?php endif;?>
