...
for email, emp in sorted(empleados.items(), key=lambda x: (x[1]['apellidos'], x[1]['nombre'])):
    ...
    for mes in meses:
        ...
        c = ws.cell(row=row, column=col+2, value=f"{pct}%" if p else '-')
        c.border = thin_border
        c.alignment = center
        if p: c.fill = get_fill(pct)
        col += 3
        total_p += p
        total_a += a
    ...
    ws.cell(row=row, column=col+1).alignment = center
    c = ws.cell(row=row, column=col+2, value=f"{pct_t}%")
    c.border = thin_border
    c.alignment = center
...
for grupo in sorted(grupos.keys()):
    ...
    for email in sorted(alumnos.keys(), key=lambda e: (alumnos[e]['apellido'], alumnos[e]['nombre'])):
        ...
        for mes in meses:
            ...
            c = ws2.cell(row=row, column=col+2, value=f"{pct}%" if p else '-')
            c.border = thin_border
            c.alignment = center
            if p: c.fill = get_fill(pct)
            col += 3
        ...
        ws2.cell(row=row, column=col+1).alignment = center
        c = ws2.cell(row=row, column=col+2, value=f"{pct_t}%")
        c.border = thin_border
        c.alignment = center
        c.fill = get_fill(pct_t)
...
for r in resumen_grupos:
    ...
    c = ws2.cell(row=row, column=3, value=f"{r['pct']}%")
    c.border = thin_border
    c.alignment = center
