...
- Última revisión: **2026-08-20** (añadidos ING-12 e ING-13 tras el caso Berta Rodríguez / Velcro; ampliados ING-2, SEC-4 y MIG-2).
...
| SEC-8 | **`askddbb.php` ejecutaba SQL por POST sin autenticacion** | Seguridad | **Alta** | 🟡 | 01-09-2026. Endpoint en la raiz del sitio que ejecutaba cualquier sentencia recibida por POST: sin `require_login()`, sin comprobacion de admin y sin `sesskey`. El control de admin estaba en `courseacuity.php` (la pagina), no en el endpoint. Ademas lo incluyen `newAcuity.php`, `cancelbymail.php`, `reminder.php` y otras, asi que un POST con `sql=` a cualquiera de ellas tambien lo ejecutaba. Salio investigando por que `courseacuity.php` decia "Guardado correctamente" y no guardaba: el endpoint **devuelve el texto del error como resultado** y el JS comprobaba `if (res)`, con lo que toda cadena de error se pintaba de verde. **DESPLEGADO Y VERIFICADO 01-09-2026**: guarda de acceso directo + `require_login()` + `is_siteadmin()` solo para `type=set`, y `if (res === true)` en los dos guardados del JS. **2a entrega**: el INSERT incluia los cursos sin configurar con `acuityid = NULL` (columna NOT NULL) y eso tumbaba el lote entero, asi que la pagina llevaba meses sin poder guardar nada. Verificado: POST sin sesion da 303, `newAcuity.php` sigue en 200 y el curso 3051 se guardo bien. Pendiente `sesskey`, sustituir el ejecutor generico y sacar los `.pre-sec8` del web root. `docs/tickets/2026-09-01-sec8-askddbb-sin-autenticacion.md` |
...
| ING-2 | **Una de cada cinco clases de 2026 no se resuelve nunca** (antes: "atascadas en Verificando") | Ingesta | **Alta** | 🔴 | **RE-DIAGNOSTICADO 07-ago-2026 → `docs/tickets/2026-08-07-ing2-clases-sin-resolver.md`.** El estado 3 **es el estado INICIAL** de toda reserva, no un atasco: hay **143 clases futuras** en estado 3 (hasta el 18-nov). Lo real: **857 clases ya impartidas sin procesar** (678 en estado 3 + 179 en estado 0) frente a 3.107 resueltas = **22%**. No cuentan en `clases_completadas` → afecta al % de asistencia FUNDAE y al cruce de facturas de profesoras. Caso verificado: Juan Antonio Muñoz (Hyatt, 7-ago 12:00, id 111016) con 29,4 min en Zoom y `zoom_duration` NULL en la BD. Son **dos problemas**: la etiqueta que ve el alumno (futuras deberían decir "Programada") y la ingesta que no baja participantes. `docs/2026-07-09-reproceso-verificando.md`. **07-ago-2026: la acumulación ha crecido en agosto** — 27-jul: 2/17 · 29-jul: 2/16 · 30-jul: 3/16 · **3-ago: 6/16** · 4-ago: 3/7 · 5-ago: 3/10 · 6-ago: 4/15. El alumno ve "Verificando asistencia" durante días y no sabe si su clase cuenta (caso Jaume Alsina, OPS-6). Investigar qué cambió a principios de agosto |
...
| ING-11 | **La integración Acuity↔Zoom cuelga de la cuenta personal de una profesora** (`paola@live.tuspeaking.com`, rol Member, tipo básico) | Ingesta | **Alta** | 🔴 | 11-ago-2026. El *"Meeting created by"* del panel de Acuity **no es el anfitrión**: es la cuenta que autorizó la integración. El mapeo calendario→usuario de Zoom funciona bien (verificadas 7 salas de 3 profesoras: cada una con su `host_email`). El problema es de **dependencia**: si esa cuenta se desactiva o pierde el token, Acuity deja de crear salas para **toda la cartera**, en silencio — y sin sala no hay enlace para el alumno ni asistencia que bajar. **Owner del Zoom: `hfernandez@tuspeaking.com`.** Arreglo: usuario de servicio con rol Admin (o el Owner) y reautorizar. ⚠️ Fuera de horario de clases y **exportando antes el mapeo calendario→usuario**. `docs/tickets/2026-08-11-ing11-integracion-acuity-zoom-cuenta-profesora.md` |
| ING-12 | **El informe del alumno da cifras imposibles: completadas > total y pendientes negativas** | Ingesta | **Alta** | 🔴 | 20-ago-2026. Visto en Nieves Sancho (449, E2Y): cursos con `clases_total = 0` y 8/16 completadas; 2978 con 35/65/−30. `mdl_i3code_acuityZoom_informe` **se regenera en cada sync**, así que el cálculo está mal HOY. Es la tabla de la que come **Success**, o sea el panel que ve RRHH del cliente — y Nieves es justo la responsable de RRHH de E2Y. **Alcance sin barrer** (salió por casualidad, revisando otra cosa). No tocar los datos: el arreglo va en la consulta que la genera. `docs/tickets/2026-08-20-ing12-informe-cifras-imposibles.md` |
| ING-13 | La ingesta cuenta las **reconexiones del profesor como participantes distintos** | Ingesta | Media | 🔴 | 20-ago-2026. Fila 108691 (Berta Rodríguez, 10/08): `zoom_participants = 5` y eran **5 sesiones solapadas de la misma profesora**. `zoom_participants` guarda el nº de filas del report de Zoom, no de personas. Una conexión inestable infla el contador y hace parecer concurrida una clase en la que solo estaba el profesor. `docs/tickets/2026-08-20-ing13-participantes-reconexiones.md` |
| ING-14 | **El 85% de las clases marcadas como "reprogramadas" nunca se movieron** | Ingesta | Media | 🔴 | 01-09-2026. `mdl_i3code_acuityZoom`: **2.949 filas con `acuity_rescheduled = 1`, de las cuales 2.505 tienen `acuity_original_datetime` IDENTICO a `acuity_datetime`**. El alumno ve en Mis Clases "Reprogramada desde" con la misma fecha y hora que la propia clase, que no significa nada. Causa en `newAcuity.php` (~linea 252): la rama de reprogramacion pone `acuity_rescheduled = 1` **incondicionalmente**, sin comprobar que la fecha haya cambiado. El `COALESCE` de `acuity_original_datetime` esta bien; el problema es que se entra en esa rama tambien cuando Acuity reenvia el mismo evento o cuando se **reprocesa** una cita con `newAcuity.php?id=`, que es el metodo de recuperacion estandar del runbook (ING-10 reproceso 28 reservas asi). Arreglo: comparar la fecha nueva con la existente y solo marcar `rescheduled` si difieren. ⚠️ **No tocar en caliente**: es el webhook, y ING-9 e ING-10 salieron de editarlo. Detectado en el caso de Daniel Medina (Capitole), al corregir dos ausencias falsas. |
...
| SEC-4 | Rotar API key Acuity + secret Zoom (llevan tiempo en el server) | Seguridad | **Alta** | 🔴 | Ver `TICKET-SEGURIDAD`. Acuity key en histórico git (`newAcuity.php`, `admin/cli/i3code_download_zoomdata.php`, `empresas/acuity_zoom.php`). **Subida a Alta el 20-ago-2026**: la key de `acuityapi.php` se manejó en claro durante una sesión de soporte (`$userID` y `$apiKey` en las líneas 2-3 del fichero, sin ofuscar). Rotar en Acuity + sacarla a configuración. |
...
- [ ] **ING-10 verificación 11-ago** → docs/tickets/2026-08-11-ing10-verificacion-post-incidente.md
